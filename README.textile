...
- @-a@ @--auto-discovery@ := Enable auto discovery of mounted volume-ids for current instance.
...
- @--region REGION@ := Specify a different EC2 region like "eu-west-1". Defaults to $AWS_DEFAULT_REGION environment variable of "us-east-1" if empty.
...
This example saves the most recent 10 snapshots for all used volumes using auto discovery
